import 'package:vivanta_banquet/data/db/app_db.dart';
...
import 'package:vivanta_banquet/data/model/room_label.dart';
import 'package:sqflite/sqflite.dart';

class RoomDB {
  static final RoomDB _taskDb = RoomDB._internal(AppDatabase.get());

  AppDatabase _appDatabase;

  //private internal constructor to make it singleton
  RoomDB._internal(this._appDatabase);

  //static RoomDB get RoomDB => _taskDb;
  ...
  /// Inserts or replaces the room.
  Future updateTask(Tasks room, {List<int> labelIDs}) async {
    var db = await _appDatabase.getDb();
    await db.transaction((Transaction txn) async {
      int id = await txn.rawInsert('INSERT OR REPLACE INTO '
          '${Tasks.tblTask}(${Tasks.dbId},${Tasks.dbTitle},${Tasks.dbRoom},${Tasks.dbPrice},${Tasks.dbClient},${Tasks.dbProjectID},${Tasks.dbComment},${Tasks.dbDueDate},${Tasks.dbPriority},${Tasks.dbStatus})'
          ' VALUES(${room.id}, "${room.title}", "${room.room}","${room.price}","${room.client}",${room.projectId},"${room.comment}", ${room.dueDate},${room.priority.index},${room.tasksStatus.index})');
